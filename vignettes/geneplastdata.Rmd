...
orthologous groups (OG) distribution in a given species tree.
This supporting package provides datasets obtained and processed from different 
orthologous databases for use in geneplast evolutionary analyses.
...
In this section, we reproduce a case study from [geneplast vignette](https://www.bioconductor.org/packages/release/bioc/vignettes/geneplast/inst/doc/geneplast.html#map-rooting-information-on-ppi-networks) using distinct datasets.

The next steps show how to retrieve the data from AnnotationHub, run geneplast rooting analysis, and transfer its results to a graph model. We annotate each dataset with ENTREZ identificators to match the PPI network provided in the example.
...
load(meta[["AH83116"]])
```
3 - Annotate `cogdata` with ENTREZ identificators using [this file](https://string-db.org/mapping_files/entrez/human.entrez_2_string.2018.tsv.gz) obtained from STRING.

```{r, eval=FALSE}
library(dplyr)
library(tidyr)
library(readr)
human_entrez_2_string_2018 <- read_delim("processed_human.entrez_2_string.2018.tsv", 
    delim = "\t", escape_double = FALSE, 
    col_names = FALSE, trim_ws = TRUE, skip = 1)

names(human_entrez_2_string_2018) <- c("ssp_id", "gene_id", "protein_id")

human_entrez_2_string_2018 <- human_entrez_2_string_2018 %>% separate_rows(gene_id) %>% separate(protein_id, c(NA, "protein_id"))

cogdata <- cogdata %>% separate(protein_id, c(NA, "protein_id")) %>% left_join(human_entrez_2_string_2018)


```

4 - Create an object of class 'OGR' for a reference 'spid'.
...
5 - Run the `groot` function and infer the evolutionary roots. 

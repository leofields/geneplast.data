...
2 - Create a `phylo` object using the function `make_phylo_tree` provided by this package. It accepts two input formats: [`sspids`] or [`newick`].
```{r eval=FALSE}
library(geneplast.data)
```

2.1 - From a list of species identifiers (NCBI taxon ID):
```{r eval=FALSE}
phyloTree <- make_phylo_tree(sspids = ssp_ids)
```

2.1 - From your own phylogenetic tree in `newick` file format:
```{r eval=FALSE}
phyloTree <- make_phylo_tree(newick = "path/to/newick_tree.nwk")
```
*Note: the tips' labels from the custom tree should be listed in 'cogdata' object to properly perform the analysis.*

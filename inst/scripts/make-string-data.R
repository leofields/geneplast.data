...
###############################################################################
#                                                                             #
# Script for creating geneplast objects from STRING database                  #
#                                                                             #
...
if (!file.exists("working_directory/string/species.v11.0.txt")) {
  download.file(
    "https://stringdb-static.org/working_directory/species.v11.0.txt",
    "working_directory/string/species.v11.0.txt"
  )
}
...
write(string_eukaryotes[["ncbi_name"]],"working_directory/string/string_eukaryotes.txt")
###############################################################################
...
# choose the file string_eukaryotes.txt and click the 'Upload' button.        #
#                                                                             #
...
if (!file.exists("working_directory/string_eukaryotes.nwk")) {
...
timetree_newick <- read.tree("working_directory/string_eukaryotes.nwk")
...
write.csv(ncbi_to_timetree,file="working_directory/string/ncbi_to_timetree.csv", row.names = FALSE)
###############################################################################
...
ncbi_to_timetree <- read_csv("working_directory/string/ncbi_to_timetree.csv")
...
####### COGDATA ####
library(data.table)
cogdata <- as.data.frame(fread("working_directory/string/COG.mappings.v11.0.txt.gz"))
...
####### SSPIDS ####
sspids <- string_eukaryotes[,c("taxid", "string_name")]
...
save(cogdata,
     ...
     cogids,
     phyloTree,
     file = "inst/extdata/gpdata_string_v11.RData",
     compress = "xz")
